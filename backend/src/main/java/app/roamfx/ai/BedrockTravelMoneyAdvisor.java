...
@Service
@ConditionalOnProperty(name = "app.aiProvider", havingValue = "bedrock")
public class BedrockTravelMoneyAdvisor implements TravelMoneyAdvisor {
  ...
  public BedrockTravelMoneyAdvisor(TravelMoneyPromptTemplate promptTemplate) {
    this.promptTemplate = promptTemplate;
  }

  @Override public TravelMoneyPlan plan(TravelMoneyPlanRequest request) {
    var prompt = promptTemplate.build(request);
    // TODO: Call AWS Bedrock Converse API with prompt, parse structured JSON, validate compliance fields, and return typed plan.
    ...
    return new TravelMoneyPlan(plan.recommendedCashAmount(), plan.recommendedCardAmount(), plan.suggestedDenominations(),
      ...
      plan.disclaimer(), "AWS Bedrock placeholder");
  }
}
